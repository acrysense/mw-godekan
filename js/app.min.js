...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (teamCard) {
    teamCard.forEach(function (card) {
      var close = function close() {
        teamCard.forEach(function (child) {
          return child.classList.remove('is--active');
        });
      };
      var itemChecker = useItemChecker([card.parentNode], close);
  ...
  // team
  var teamWrapper = document.querySelector('.team__wrapper');
  var teamShowMore = document.querySelector('.team__more');
  function setInitialHeight() {
    var card = document.querySelector('.team__item');
    var cardHeight = card.offsetHeight;
    if (window.innerWidth <= 743) {
      teamWrapper.style.maxHeight = "".concat((cardHeight + 20) * 2.5, "px");
    } else if (window.innerWidth <= 1023 && window.innerWidth >= 744) {
      teamWrapper.style.maxHeight = "".concat((cardHeight + 20) * 3.5, "px");
    } else if (window.innerWidth >= 1024) {
      teamWrapper.style.maxHeight = 'none';
    }
  }
  if (teamShowMore) {
    teamShowMore.addEventListener('click', function (event) {
      event.preventDefault();
      var totalHeight = Array.from(teamWrapper.children).reduce(function (acc, card) {
        return acc + card.offsetHeight;
      }, 0);
      teamWrapper.style.maxHeight = "".concat(totalHeight, "px");
      setTimeout(function () {
        teamWrapper.style.maxHeight = 'none';
      }, 500);
      teamShowMore.style.display = 'none';
    });
  }
  if (teamWrapper && teamShowMore) {
    setInitialHeight();
  }
  window.addEventListener('resize', function () {
    if (teamWrapper && teamShowMore) {
      if (teamWrapper.style.maxHeight !== 'auto') {
        setInitialHeight();
      }
    }
  });
  ...
  // approach
  var approachItems = document.querySelectorAll('.approach__item');
  var handleScroll = function handleScroll() {
    var viewportHeight = window.innerHeight || document.documentElement.clientHeight;
    var viewportCenter = viewportHeight / 2;
    approachItems.forEach(function (item) {
      var rect = item.getBoundingClientRect();
      var itemCenter = rect.top + rect.height / 2;
      var distanceFromCenter = Math.abs(viewportCenter - itemCenter);
      var maxDistance = viewportHeight / 2;
      var adjustedRatio = Math.max(0, 1 - distanceFromCenter / maxDistance);
      var opacity = 0.1 + adjustedRatio * 0.9;
      var scale = 0.95 + adjustedRatio * 0.05;
      item.style.opacity = opacity;
      item.style.transform = "scale(".concat(scale, ")");
    });
  };
  ...
  // about slider
  var aboutSlider = document.querySelector('.about__slider .swiper');
  if (aboutSlider) {
    var updatePagination = function updatePagination() {
      paginationItems.forEach(function (item, index) {
        if (index === myAboutSlider.realIndex) {
    ...
    var myAboutSlider = new Swiper(aboutSlider, {
      slidesPerView: 1,
      loop: true,
    ...
    var paginationItems = document.querySelectorAll('.about__slider .custom-pagination-bullet');
    paginationItems.forEach(function (item) {
      item.addEventListener('click', function () {
        var index = item.getAttribute('data-index');
        myAboutSlider.slideTo(index);
      });
    });
    updatePagination();
  }
